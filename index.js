const {
  ...
  const { File } = require('megajs') // Keep this if other parts use it, otherwise remove if only used for session
 ...
 // ------------------------------------------ 
  ...
  //===================SESSION-AUTH============================
// --- REMOVED MEGA SESSION LOADING ---
// The session loading will now happen in connectToWA using loadSession if needed.
...
const whatsappChannelLink = "https://whatsapp.com/channel/0029VasHgfG4tRrwjAUyTs10";
  ...
  // 1. Load Session if SESSION_ID is provided and creds.json doesn't exist yet
  if (config.SESSION_ID && !fs.existsSync(path.join(__dirname, 'session', 'creds.json'))) {
      try {
          console.log("Session ID detected. Attempting to load session...");
          await loadSession(config.SESSION_ID);
      } catch (e) {
          console.error("Fatal Error loading session:", e.message);
          // If session loading fails fatally, we might stop or proceed hoping for QR code
          if (!config.SESSION_ID) return console.log('Please add your session to SESSION_ID env !!');
      }
  } else if (!config.SESSION_ID) {
      return console.log('Please add your session to SESSION_ID env !! (Required for QR code generation/reconnection)');
  }


  const { state, saveCreds } = await useMultiFileAuthState(__dirname + '/session/') // Ensure this path matches where loadSession saves it if it saves it directly to creds.json
  var { version } = await fetchLatestBaileysVersion()

  const conn = makeWASocket({
          logger: P({ level: 'silent' }),
          printQRInTerminal: true, // Set to true if session loading fails and QR is needed
  ...
  connectToWA()
      ...
      console.log("Logged out. Please re-scan QR code or update session ID.");
  }
