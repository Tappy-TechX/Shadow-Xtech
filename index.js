...
// Global flag to ensure channel follow happens only once per bot instance
    ...
    conn.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            if (lastDisconnect.error.output.statusCode !== DisconnectReason.loggedOut) {
                ...
                connectToWA(); // Re-call connectToWA on close
            } else {
                console.log('[🔴] WhatsApp logged out. Please re-scan QR code.');
                // Optionally handle logout, e.g., delete session files
            }
            ...
            // --- Auto-Follow Specific WhatsApp Newsletter ---
            const channelJID = '120363369453603973@newsletter';
            if (!hasFollowedChannel) { // Use the global flag
                try {
                    await conn.newsletterFollow(channelJID);
                    console.log("[🟠] Successfully followed WhatsApp newsletter:", channelJID);
                    hasFollowedChannel = true; // Set the global flag
                } catch (e) {
                    console.error("[🔴] Failed to auto-follow WhatsApp newsletter:", e);
                    // If it fails, we might want to retry on next connection, so don't set hasFollowedChannel to true here.
                }
            }
            // ------------------------------------------------
...
> _© *Powered By Black-Tappy*_`;
    ...
    }); // End of conn.ev.on('connection.update')
