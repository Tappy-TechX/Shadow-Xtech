...
if (!fs.existsSync(__dirname + '/sessions/creds.json')) {
  ...
  filer.download((err, data) => {
    ...
    fs.writeFile(__dirname + '/sessions/creds.json', data, () => {
      console.log("📥 Session loaded successful ✔️");
    });
  });
}
...
async function connectToWA() {
  ...
  // --- Wrap sendMessage globally ---
  const originalSendMessage = conn.sendMessage.bind(conn);
  conn.sendMessage = async (jid, content, options = {}) => {
    // Apply the transformMessage function to the content before sending
    const transformedContent = transformMessage(content);
    return originalSendMessage(jid, transformedContent, options);
  };
  // ---------------------------------
...
│   Click [*Here*](254756360306)
├─ ⭐ *Give Us a Star:*
│   Star Us [*Here*](https://github.com/Tappy-TechX/Shadow-Xtech)!
╰─🛠️ *Prefix:* \`${prefix}\`
