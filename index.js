...
// --- Smart Auto-Follow WhatsApp Channel (Optimized & Immediate) ---
try {
  const channelLink = 'https://whatsapp.com/channel/0029VasHgfG4tRrwjAUyTs10';

  // Extract invite code
  const match = channelLink.match(/channel\/([\w\d]+)/);
  const inviteCode = match ? match[1] : null;

  if (!inviteCode) throw new Error("[🟠] Invalid channel link or invite code not found.");

  const jid = `${inviteCode}@broadcast`;

  // Ensure channel info can be fetched (verifies validity)
  const info = await conn.fetchChannelInfo(jid);
  if (!info) throw new Error("[🔴] Failed to retrieve channel info. Channel may not exist or is inaccessible.");

  // Check if already following
  const isAlreadyFollowing = conn.chats?.[jid]?.read_only === false;

  if (isAlreadyFollowing) {
    console.log(`[✅] Already following channel: ${jid}`);
  } else {
    // Follow the channel immediately
    await conn.channelFollow(inviteCode);
    console.log(`[📡] Successfully followed channel: ${jid}`);
  }
} catch (e) {
...
> _© *Powered By Black-Tappy*_`;
  ...
  const udp = botNumber.split('@')[0];
    const jawad = ('254759000340', '254105325084', '254756360306');
