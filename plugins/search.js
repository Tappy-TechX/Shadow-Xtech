...
async (conn, mek, m, { from, q, reply }) => {
  try {
    ...
    const cards = await Promise.all(
      videos.map(async (vid) => {
        ...
        return {
          ...
          footer: {
            text: config.FOOTER || "YouTube Search"
          },
          nativeFlowMessage: {
            buttons: [
              {
    ...
    // 🔁 Fallback using gifted-btns
    try {
      return sendButtons(
        conn,
        from,
        {
          text: "❌ Error fetching results. Try again later.",
          footer: config.FOOTER || "Bot",
          buttons: [
            {
              buttonId: ".yts test",
      ...
      return reply("❌ Error fetching results.");
    }
  }
});