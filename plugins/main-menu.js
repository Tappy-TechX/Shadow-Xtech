...
const getRandomMenuImage = () => {
    const randomIndex = Math.floor(Math.random() * MENU_IMAGES.length);
    return MENU_IMAGES[randomIndex];
};
...
async (conn, mek, m, { from, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply }) => {
    try {
        const selectedImageUrl = getRandomMenuImage(); // Get a random image URL
...
> ${config.DESCRIPTION}`;
        ...
        await conn.sendMessage(
            from,
            {
                image: { url: selectedImageUrl }, // Use the randomly selected image URL
                caption: dec,
                contextInfo: {
